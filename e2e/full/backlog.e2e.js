...
describe('backlog', function() {
    ...
    it('move to current sprint button', async function() {
        ...
        let draggedText = await dragElement.element(by.binding('us.subject')).getText();

        $('#move-to-current-sprint').click();

        let sprint = $$('div[tg-backlog-sprint="sprint"].sprint-open').last();

        let sprintTexts = await sprint.$$('span[tg-bo-bind="us.subject"]').getText();

        expect(sprintTexts.indexOf(draggedText)).to.be.not.equal(-1);
    });
    ...
    it('select us with SHIFT', async function() {
        ...
        let count = await $$('.backlog-table-body input[type="checkbox"]:checked').count();
    ...
    describe('filters', function() {
        ...
        it('filter by ref', async function() {
            ...
            let htmlChanges = await utils.common.outerHtmlChanges('.backlog-table-body');
            let firstUs = await $$('.backlog-table-body > div span[tg-bo-ref]').first().getText();
            let ref = firstUs.replace('#', '');
        ...
        it('filter by status', async function() {
            ...
            newUsCount = await $$('.backlog-table-body > div').count();
